...
const Player = ({
    ...
    const keyDownHandler = (event) => {
      ...
      switch (true) {
        ...
        case event.key == "q" || event.keyCode == 37:
          if (
            checkCell([
              playerCoo.current.index.coo[0],
              playerCoo.current.index.coo[1] - 1,
            ])
          ) {
            score.current.index.score++;
            setDisplayScore(score.current.index.score);
      ...
      coo.current.index = {
        top: `${coo.current.index.y}vh`,
        left: `${coo.current.index.x}vw`,
        x: coo.current.index.x,
        y: coo.current.index.y,
      };
  ...
  return (
    <div
      id="player"
      style={{ top: coo.current.index.top, left: coo.current.index.left }}
    ></div>
  );
};
